...
const Rooms = () => {
    ...
    useEffect(() => {
        const generateRooms = async () => {
            try {
                const presidentialResponse = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/rooms/get-presidential-rooms`);
                setPresidentialRooms(presidentialResponse.data.data);

                const premiumResponse = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/rooms/get-premium-rooms`);
                setPremiumRooms(premiumResponse.data.data);

                const luxuryResponse = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/rooms/get-luxury-rooms`);

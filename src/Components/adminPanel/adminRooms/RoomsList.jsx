...
const RoomsList = () => {
    ...
    useEffect(() => {
        ...
        const fetchRoom = async () => {
            setLoading(true)
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/rooms/get-all-rooms`, {
    ...
    const handleDelete = async (roomId) => {
        try {
            const response = await axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/rooms`, {
                data: { roomId },
                withCredentials: true
            })
